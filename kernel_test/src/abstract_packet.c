...
int create_content(content_t **content, uint8_t type, uint8_t subject_len, char *subject, data_t *payload){
	...
	*content = (content_t *)malloc(sizeof(content_t));
	...
	(*content)->subject = (char *)malloc(subject_len + 1);
...
void print_content(content_t *content){
	if(content!=NULL){
		printf("content : ");
		printf("type : %d ", content->type);
		printf("subject_len : %d ", content->subject_len);
		printf("subject : %s\n", content->subject);
...
void print_abstract_packet(abstract_packet_t *packet){
	...
	int src = ntohl(packet->src);
    memcpy(&addr, &packet->src, sizeof(struct in_addr));
	printf("Src: %s/%d ", inet_ntoa(addr), packet->src_bm);

	int dst = ntohl(packet->dst);
    memcpy(&addr, &packet->dst, sizeof(struct in_addr));
	printf("Dst: %s/%d ", inet_ntoa(addr), packet->dst_bm);

	printf("Sport: %d ", ntohs(packet->sport));
	printf("Dport: %d\n", ntohs(packet->dport));
